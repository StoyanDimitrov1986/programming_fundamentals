...
<div class="test-form">
    ...
    foreach ($model->testQuestions as $key => $testQuestion) {
        ...
        echo $form->field($answer, '[' . $testQuestion->id . ']' . 'test_question_id')
            ->hiddenInput(['value' => $testQuestion->id, 'disabled' => true])
            ->label(false);

        $evaluation = $evaluations[$answer->id];
    ...
    <?php if ($mode !== 'view'): ?>
        <div class="form-group">
            <?= Html::submitButton('Evaluate', ['class' => 'btn btn-success']) ?>
        </div>
    <?php endif ?>

    <?php if ($mode === 'view'): ?>
        <?= Html::a('Back', ['test/view-waiting'], ['class' => 'btn btn-success']) ?>
        <?= Html::a('Update', ['test/update-evaluation', 'id' => $model->id], ['class' => 'btn btn-primary']) ?>
    <?php endif ?>
